...
float aRefMid = 0.0;
float DACaRef = 3.3;
float DCoffset = 0.07;    //measured analytically
  ...
  switch (waveType) {
    ...
    case (0):
    {
      ...
      for (int32_t i = 0; i < samples; i++) {
        ...
        Serial.println(-(((value-ref) * aRef / 65535.0)/aRefMid)*gain, 6);    // ratio, value/2^16, is the percent of ADC reference... * aRef (ADC Reference Voltage) == Voltage measured
        while (timer < samplingDelay); // wait
        timer = timer - samplingDelay;
      }
    ...
    case (1):
    {
      ...
      for (int32_t i = 0; i < samples; i++) {
        ...
        Serial.println(-((value * aRef / 65535.0-aRef/2)/aRefMid)*gain, 6);    // 
    ...
    case (2): // triangle wave
    {
      ...
      while (j < iterations) {
        ...
        for (int32_t i = 0;  i < flipSample; i++) {
          ...
          voltMark += scanRate/(1000.0*sampleRateFloat);          //  V/s / (samples/second) >> volts/sampl
    ...
    case (3): 
    {
      ...
      for (int32_t i = 0; i < samples; i++) {
        ...
        Serial.println(-(((value) * aRef / 65535.0 - aRefMid)/aRefMid)*gain, 6);    // ratio, value/2^16, is the percent of ADC reference... * aRef (ADC Reference Voltage) == Voltage measured
        while (timer < samplingDelay); // wait
        timer = timer - samplingDelay;
      }
